...
int main(int argc, char* argv[]) {
  ...
  TEST_NEW("hash_map_insert_ptr_test") {
    hash_map map;
    hash_map_create(&map);
    ASSERT_TRUE(hash_map_insert_ptr(&map, (void*)1, (void*)3));
    hash_map_bucket* it = hash_map_find_ptr(&map, (void*)1);
    ASSERT_TRUE(it != NULL);
  ...
  TEST_NEW("hash_map_insert_str_test") {
    hash_map map;
    hash_map_create(&map);
    ASSERT_TRUE(hash_map_insert_str(&map, "a", (void*)1));
    ASSERT_TRUE(hash_map_insert_str(&map, "a", (void*)2));
    ASSERT_TRUE(hash_map_insert_str(&map, "a", (void*)3));
  ...
  TEST_NEW("hash_map_iter_test") {
    ...
    hash_map_create(&map);
    for (int i = 0; i < 64; i++) {
      ASSERT_TRUE(hash_map_insert_ptr(&map, (void*)i, (void*)i));
    }
    hash_map_iter it;
